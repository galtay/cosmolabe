REQUIRED_CPDICT_KEYS = [
    'Omega_m', 'Omega_l', 'Omega_b', 'h', 'sigma_8', 'n_s', 'Y_p']
...
class Cosmology(object):
    ...
    def __init__( self, cpdict, verbose=False, zlo=0.0, zhi=200.0, Nz=500 ):
        """
        :param cpdict: A dictionary of cosmological parameters.
          The dictionray must include the following keys,
            - ``Omega_m`` -> matter density in units of critical today
            - ``Omega_l`` -> lambda density in units of critical today
            - ``Omega_b`` -> baryon density in units of critical today
            - ``h``       -> Hubble parameter H0 = 100 h km/s/Mpc
            - ``sigma_8`` -> amplitude of fluctuations in spheres w/ R = 8 Mpc/h
            - ``n_s``     -> slope of primordial power spectrum
            - ``Y_p``     -> primordial mass fraction of helium
        ...
        #--------------------------------------------------------
        for key in REQUIRED_CPDICT_KEYS:
            if not cpdict.has_key(key):
                raise ValueError('key ({}) missing from cpdict.'.format(key))

        # transfer input parametrs
        #--------------------------------------------------------
        self.OmegaM = cpdict['Omega_m']
        self.OmegaL = cpdict['Omega_l']
        self.OmegaB = cpdict['Omega_b']
        self.h = cpdict['h']
        self.sigma8 = cpdict['sigma_8']
        self.ns = cpdict['n_s']
        self.Yp = cpdict['Y_p']
